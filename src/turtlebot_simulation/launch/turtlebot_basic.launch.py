...
from launch.substitutions import LaunchConfiguration, Command, PathJoinSubstitution, IfElseSubstitution
...
def generate_launch_description():
    ...
    # planning:=true  → loads turtlebot_planning.rviz (adds Map, /plan, /local_path, DWA trajectories)
    # planning:=false → loads turtlebot.rviz (default, no planning displays)
    declare_planning = DeclareLaunchArgument(
        'planning', default_value='false',
        description='Set true to load planning displays in RViz (Map, /plan, /local_path, DWA fan)'
    )
    declare_rviz_config = DeclareLaunchArgument(
        'rviz_config',
        default_value=IfElseSubstitution(
            LaunchConfiguration('planning'),
            if_value=PathJoinSubstitution([pkg_turtlebot_desc, 'rviz', 'turtlebot_planning.rviz']),
            else_value=PathJoinSubstitution([pkg_turtlebot_desc, 'rviz', 'turtlebot.rviz'])
        ),
        description='Full path to RViz config file; overrides the planning flag when set explicitly'
    )
